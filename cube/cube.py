...
class cube:
    ...
    def print(self):
        ...
        rows = [
            ...
            self.cube['L'][0] + self.cube['F'][0] + self.cube['R'][0] + self.cube['B'][0],
            self.cube['L'][1] + self.cube['F'][1] + self.cube['R'][1] + self.cube['B'][1],
            self.cube['L'][2] + self.cube['F'][2] + self.cube['R'][2] + self.cube['B'][2],
    ...
    def new_cube(self):
        ...
        """
        self.cube = {
            face: [[self.colors[self.faces.find(face)] + str(i + 1 + 3 * j) for i in range(3)] for j in range(3)]
  ...
  "F6": "R6",
    ...
    def id_position(self,face,position):
        ...
        if position == (1,1):
            ...
            retval['opposite_face_color'] = self.color_of_position(retval['opposite'][0],(1,1))
            ...
            for mate in retval['mates']:
                ...
                retval['mate_face_color'].append(self.color_of_position(mate[0],(1,1)))
        else:
            retval['type'] = 'unkown'
    ...
    def check_white_cross(self):
        ...
        self.orient_cube()
        for square in [self.cube['D'][0][1],self.cube['D'][1][0],self.cube['D'][1][2],self.cube['D'][2][1]]:
            if square[0] != 'W':
                return False
    ...
    def place_white_edge(self,face,position,recursion_level=-1):
        ...
        if face in 'UD':
            ...
            if face == 'D':
                ...
                if data['color'] == data['face_color'] and  data['opposite_color'] == data['opposite_face_color']:
                    return True
                if moves > 0:
                    command_string = f"{data['opposite'][0]},"*2 + "U',"*moves + f"{self.find_center(data['opposite_color'])},"*2
                ...
                if moves > 0:
                    command_string = "U',"*moves + f"{self.find_center(data['opposite_color'])},"*2
                    self.run_command_string(command_string)
                    face, position = self.get_white_edge_by_color(data['opposite_color'])
                    return self.place_white_edge(face,position,recursion_level)
                else:
    ...
    def check_white_corners(self):
        ...
        for square in [self.cube['D'][0][0],self.cube['D'][0][2],self.cube['D'][2][0],self.cube['D'][2][2]]:
            if square[0] != 'W':
                return False
    ...
    def place_white_corner(self,face,position,recursion_level=-1):
        ...
        if face == 'D':
            ...
            if face in self.sides:
                ...
                logging.debug(f"Found a mate: {mate_face} {mate_position} {corner_data}")
                if position == (0,0):
                    self.run_command_string(self.transpose_command("L,L,U',L,L,U,L,L",face))
                if position == (0,2):
                    sides = 'FLBR'
                    ...
                    self.run_command_string(self.transpose_command("L,L,U',L,L,U,L,L",tmp_face))
                else:
                    pass
                face, position = self.get_white_corner_by_color(colors)
                return self.place_white_corner(face,position,recursion_level)
    ...
    def hover_white_corner(self,face,position):
        ...
        while not found:
            ...
            if not found:
                ...
                if face_idx == 4:
                    face_idx = 0
                logging.debug(f" Setting face from {face} to {sides[face_idx]}:{face_idx}")
                face = sides[face_idx]
                corner_data = self.id_position(face,position)
    ...
    def solve_second_layer(self):
        ...
        while not self.check_second_layer():
            ...
            for face, position in edges:
                ...
                if edge_data['color'] not in 'WY' and edge_data['opposite_color'] not in 'WY':
                    # skip edges that are already in the right place
                    if edge_data['color'] != edge_data['face_color'] or edge_data['opposite_color'] != edge_data['opposite_face_color']:
                        if position == (0,1) and face != 'U':
                            ...
                            while edge_data['color'] != edge_data['face_color']:
                                ...
                                face = sides[face_idx]
                                edge_data = self.id_position(face,position)
                            if (edge_data['color'],edge_data['opposite_color']) in (('O','G'),('G','R'),('R','B'),('B','O')):
                                # break left
                                self.run_command_string(self.transpose_command("U',L',U,L,U,F,U',F'",face))
                            else:
                                # break right
                                self.run_command_string(self.transpose_command("U,R,U',R',U',F',U,F",face))
                            break
                        elif face == 'U':
                            pass
                        else:
    ...
    def check_second_layer(self):
        ...
        for side in self.sides:
            ...
            for square in [self.cube[side][1][0],self.cube[side][1][1],self.cube[side][1][1][0],self.cube[side][1][2]]:
                if square[0] != color:
                    return False
        return True
    ...
    def check_yellow_cross(self):
        ...
        self.orient_cube()
        for square in [self.cube['U'][0][1],self.cube['U'][1][0],self.cube['U'][1][2],self.cube['U'][2][1]]:
            if square[0] != 'Y':
                return False
        return True
        ...
        while (yellow_edges := self.count_yellow_edges()) < 4:
            ...
            if target in edge_colors+edge_colors:
                ...
                while self.cube['F'][0][1][0] != self.cube['F'][1][1][0]:
                    logging.debug("solve_yellow_edges: {self.cube['F'][0][1][0]} != {self.cube['F'][1][1][0]}")
                    self.run_command_string('U')
            else:
    ...
    def solve_yellow_corners(self):
        ...
        while True:
            ...
            if len(matched_corners) == 1:
                corner_data = self.id_position(matched_corners[0][0],matched_corners[0][1])
                target = ('F',(0,2))
    ...
    def check_yellow_corners(self):
        ...
        self.orient_cube()
        # are all the white corners facing down
        for square in [self.cube['U'][0][0],self.cube['U'][0][2],self.cube['U'][2][0],self.cube['U'][2][2]]:
            if square[0] != 'Y':
                return False
        # are all the white corners in the right slots
        corners = self.find_corner_pieces('Y')
