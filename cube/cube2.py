...
class Cube:
        ...
        if position in [1,3,7,9]:
            for corner in self.cube['corners']:
                for square in corner:
                    if (square['position'] == position and
                        square['center'] == self.cube['faces'][face]):
                        return corner
        # edges
    ...
    def print(self,label=None):
        ...
        logging.debug("cube%s.print()",self.type)
        """
    ...
    def new_cube(self):
        ...
        for edge_a, edge_b in edge_map:
            self.cube['edges'].append([
                {"color": edge_a[0], "center": edge_a[0], "position": edge_a[1], "start": edge_a[2]},
                {"color": edge_b[0], "center": edge_b[0], "position": edge_b[1], "start": edge_b[2]}
            ])
        ...
        for edge_a, edge_b, edge_c in corner_map:
            self.cube['corners'].append([
                {"color": edge_a[0], "center": edge_a[0], "position": edge_a[1], "start": edge_a[2]},
                {"color": edge_b[0], "center": edge_b[0], "position": edge_b[1], "start": edge_b[2]},
                {"color": edge_c[0], "center": edge_c[0], "position": edge_c[1], "start": edge_c[2]}
            ])
        ...
        try:
            ...
            for face in self.faces:
                for position in range(1,10):
                    save_data[f'{face}{position}'] = self.get_square_label(face,position)
    ...
    def load(self, filename):
        ...
        Returns:
            None
        """
        logging.warning("load(%s)",filename)
        ...
        try:
            ...
            for position, value in data.items():
                ...
                if int(current_position) != 5:
                    for piece in self.get_pieces(original_color):
                        for square in piece:
                            if (square['color'] == original_color and
                                square['start'] == int(original_position)):
    ...
    def scramble(self,moves=40):
        ...
        """
        logging.debug("cube.scramble(%)",moves)
    ...
    def orient_cube(self,front='R',down='W'):
        ...
        while self.get_square('F',5)['color'] != front:
            sentinel += 1
            if sentinel > 4:
                logging.warning("cube2.orient_cube(%s%s) no match in all four sides",front,down)
                break
            self.rotate_cube()
    ...
    def whole_cube(self,faces,clockwise=True):
        ...
        for idx in range(len(faces)):
            self.cube['faces'][faces[idx]] = new[idx]
        if 'U' not in faces:
    ...
    def make_move(self, move):
        ...
        Returns:
            None
        """
        logging.warning("cube2.make_move(%s)",move)
    ...
    def rotate_axis(self, face, colors, positions, clockwise):
        ...
        for piece in self.get_pieces(self.cube['faces'][face], ['edges', 'corners'], True, 'center'):
            for square in piece:
                if square['center'] in colors:
                    idx = (colors.find(square['center']) + 1) % len(colors)
                    next_color = colors[idx]

                    if (positions in [[1, 4, 7], [3, 6, 9]] and
                            self.cube['faces']['B'] in [square['center'], next_color]):
                        self.update_square_position(square, positions, colors, next_color)
                    elif square['position'] in positions:
                        square['center'] = next_color

    def update_square_position(self, square, positions, colors, next_color):
        """
        ...
        if (square['center'] == self.cube['faces']['B'] and
                square['position'] in alt_positions):
            position_map = dict(zip(alt_positions, positions))
        elif (next_color == self.cube['faces']['B'] and
              square['position'] in positions):
    ...
    def rotate_y_axis(self,plane,clockwise=True):
        ...
        if plane == 2:
            ...
            logging.debug("cube2.rotate_y_axis(%s, %s):  Invalid plane %s",plane,clockwise,plane)
    ...
    def rotate_x_axis(self,plane,clockwise=True):
        ...
        if plane == 2:
            ...
            logging.debug("cube2.rotate_x_axis(%s, %s):  Invalid plane %s",plane,clockwise,plane)
    ...
    def rotate_z_axis(self,plane,clockwise=True):
        ...
        if plane == 2:
            ...
            logging.debug("cube2.rotate_z_axis(%s, %s):  Invalid plane %s",plane,clockwise,plane)
    ...
    def superflip(self):
        ...
        """
        self.run_command_string("R,L,U,U,F,U',D,F,F,R,R,B,B,L,U,U,F',B',U,R,R,D,F,F,U,R,R,U")

    """
    Search and Identify functions
    """

    def get_pieces(self,colors,piece_types=['corners','edges'],inclusive=True,match='color'):
        """
        Find pieces that match criteria
        ...
        for piece_type in piece_types:
            for piece in self.cube[piece_type]:
                ...
                if found:
                    pieces.append(piece)
        if not inclusive: # if not inclusive, we don't want pieces that have another color
            real_pieces = []
            for piece in pieces:
                keep = True
                for square in piece:
    ...
    def place_white_edge(self,edge):
        ...
        if edge[0]['color'] == 'W':
            ...
            logging.warning("cube2.place_white_edge: edge is not white: %s",edge)

        sentinel = 0
        while (white_square['color'] != white_square['center'] or
            color_square['color'] != color_square['center']):
            sentinel+=1
            if sentinel > 4:
            ...
            if white_square['center'] == 'W' and color_square['color'] != color_square['center']:
            ...
            if white_square['center'] == 'Y':
                ...
                self.run_command_string(self.face_of_color(color_square['center']))
                self.run_command_string(self.face_of_color(color_square['center']))
        if (white_square['color'] == white_square['center'] and
            color_square['color'] == color_square['center']):
            # already aligned
            return True
        return False
    ...
    def place_white_corner(self,piece):
        ...
        while not self.check_white_corner(piece):
            ...
            if sentinel > 10:
                ...
                break
            if white_square['center'] == 'Y':
                for idx in list(range(len(others))): # move to top right
                    idx2 = (idx+1) % 2
                    self.orient_cube(others[idx2]['center'])
                    while others[idx2]['color'] != others[idx]['center']:
                        self.run_command_string("U")
                    self.orient_cube(others[idx2]['center'])
                    ...
                    if square['color'] != square['center']:
                        if square['color'] != square2['center']:
                            ...
                            if square['position'] == 6:
                                ...
                                self.orient_cube(square2['center'])
                            #self.print()
                            self.run_command_string("U,R,U',R',U',F',U,F,U,U,U,R,U',R',U',F',U,F")
        ...
        while len(correct_corners) < 4:
            ...
            if len(correct_corners) != 4:
                if len(correct_corners) == 1:
                    ...
                    print(cube)
                    logging.warning("solve_yellow_corners: wtf? len(correct_corners) == %s",
                        len(correct_corners))
