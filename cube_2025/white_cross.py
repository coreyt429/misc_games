...
def run_cube_states(state_string, move_limit=2):
    ...
    while heap:
        ...
        if len(heap) % 1000 == 0:
            print(f"heap: {len(heap)}, moves: {moves}, move_limit: {move_limit}, saved: {len(already_seen)}, state_string: {state_string}")
        ...
        heappush(heap, (moves + 1, str(cube), 'X', True, history + ((face, clockwise),)))
    return already_seen
